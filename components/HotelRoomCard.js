...
export default function HotelRoomCard({ room, featured = false }) {
  ...
  return (
    <article className={`theme-card-light flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-[rgba(18,52,60,0.16)] ${featured ? "shadow-[0_28px_54px_rgba(18,52,60,0.12)]" : "shadow-sm"}`}>
      <Link href={roomHref} className="relative block h-56 overflow-hidden bg-white sm:h-64">
        ...
        <div className="absolute inset-x-0 top-0 flex items-center justify-between gap-3 p-4">
          ...
          <span className="rounded-full bg-[rgba(17,124,146,0.92)] px-3 py-1 text-[0.68rem] font-semibold uppercase tracking-[0.2em] text-white shadow-sm">
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm leading-7 theme-muted-page">{room.description}</p>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-3 sm:mt-auto sm:grid-cols-2">
          <Link href={roomHref} className="theme-card-light inline-flex min-h-[3rem] items-center justify-center rounded-[1rem] px-4 py-3 text-sm font-semibold text-[var(--foreground-strong)] shadow-sm">
            View room
          </Link>
          <Link href={bookingHref} className="theme-button-accent inline-flex min-h-[3rem] items-center justify-center rounded-[1rem] px-4 py-3 text-sm font-semibold">
            Book this room
          </Link>
        </div>
      </div>
