...
async function getBranding() {
  try {
    ...
    return {
      ...
      contactEmail: store?.email || process.env.EMAIL_USER || process.env.SMTP_FROM || "",
    ...
    return {
      ...
      contactEmail: process.env.EMAIL_USER || process.env.SMTP_FROM || "",
      website: "",
...
export async function sendOrderPlacedEmail(order) {
  return sendOrderLifecycleEmail({
    ...
    intro: "Your items have been reserved successfully. Payment will be confirmed manually by the team, delivery fee is currently waived on web orders, and any active campaign promotions are already reflected in the prices below.",
