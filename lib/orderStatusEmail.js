...
const DEFAULT_BRAND_NAME = "St Michael's Food & Drinks Warehouse";
...
function resolveLocationContactEmail(store, order) {
  const locations = Array.isArray(store?.locations) ? store.locations : [];
  const normalizedLocationName = String(order?.locationName || "").trim().toLowerCase();

  const exactLocation = locations.find((location) => {
    return String(location?.name || "").trim().toLowerCase() === normalizedLocationName;
  });

  if (String(exactLocation?.email || "").trim()) {
    return String(exactLocation.email).trim();
  }

  const firstLocationWithEmail = locations.find((location) => String(location?.email || "").trim());
  return String(firstLocationWithEmail?.email || "").trim();
}

async function getBranding(order) {
  try {
    const store = await Store.findOne({}).lean();
    const locationEmail = resolveLocationContactEmail(store, order);
    const fallbackEmail =
      locationEmail ||
      store?.email ||
    ...
    return {
      ...
      storeName: store?.storeName || store?.companyName || DEFAULT_BRAND_NAME,
    };
  } catch {
    return {
    ...
    <div style="font-family: 'Segoe UI', Arial, sans-serif; background: #f4f4f5; padding: 24px; color: #18181b;">
      <div style="max-width: 680px; margin: 0 auto; background: #ffffff; border: 1px solid #e4e4e7; border-radius: 18px; overflow: hidden; box-shadow: 0 12px 30px rgba(24, 24, 27, 0.08);">
        ...
        <div style="padding: 28px 24px 30px;">
          <p style="margin: 0 0 12px; font-size: 16px; color: #18181b;">Hi <strong>${escapeHtml(recipientName || order?.customer?.name || contact?.name || "Customer")}</strong>,</p>
          ...
          <div style="border: 1px solid #e4e4e7; border-radius: 14px; padding: 18px;">
            ...
            <div style="margin-top: 18px; border-top: 1px solid #d4d4d8; padding-top: 16px;">
              ...
              <div style="display: flex; justify-content: space-between; gap: 12px; font-size: 14px; color: #3f3f46; margin-bottom: 8px;">
                ...
                <strong>${escapeHtml(formatCurrency(totals.shippingCost))}</strong>
              </div>
...
async function sendOrderLifecycleEmail({
  ...
  const branding = await getBranding(order);
  const resolvedRecipient = recipient || order?.customer?.email || contact?.email || "";

  if (!resolvedRecipient) {
    console.warn(`Order email skipped for '${subjectLine}' because no recipient was resolved for order ${order?._id || "unknown"}.`);
    return "skipped";
  }

  if (!transporter) {
    console.warn(`Order email skipped for '${subjectLine}' because storefront mail delivery is not configured.`);
    return "skipped";
  }
...
export async function sendOrderPlacedEmail(order) {
  return sendOrderLifecycleEmail({
    order,
    subjectLine: "We have received your order",
    headline: "Your order has been received",
    intro: "Your items have been reserved successfully. Payment will be confirmed manually by the team, and any delivery fee or online campaign pricing configured in the inventory system is already reflected in the invoice below.",
    statusLabel: "Inventory Reserved",
    accentColor: "#1d4ed8",
    fulfilmentFallback: "Store Website",
  });
}

export async function sendBusinessOrderPlacedEmail(order) {
  const branding = await getBranding(order);

  return sendOrderLifecycleEmail({
    order,
    recipient: branding.businessEmail,
    recipientName: branding.storeName || "Team",
    subjectLine: "New web order received",
    headline: "A new web order is awaiting attention",
    intro: "A customer just placed a web order. Review the invoice, confirm the manual payment workflow, and continue fulfilment from the inventory system or POS.",
    statusLabel: "New Web Order",
    accentColor: "#0f766e",
    fulfilmentFallback: "Store Website",
  });
}
...
export async function sendOrderProcessingEmail(order) {
  return sendOrderLifecycleEmail({
    ...
    intro: "Your order has moved into fulfilment and is now being prepared by the team. We will notify you again when delivery has been completed.",
