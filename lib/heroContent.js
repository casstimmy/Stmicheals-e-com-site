...
function socialKey(link) {
  return [link.platform, link.url, link.handle, link.scope].map((value) => String(value || "").toLowerCase()).join("|");
}

function getScopedSocialLinks(heroes, siteKey) {
  const scopes = SITE_SOCIAL_SCOPES[normalizePublicSite(siteKey)] || SITE_SOCIAL_SCOPES[PUBLIC_SITE_KEYS.STORE];
  const socialLinksByKey = new Map();
  ...
  heroes.forEach((hero) => {
    (hero.socialLinks || [])
      ...
      .forEach((link) => {
        ...
        if (!socialLinksByKey.has(key)) {
          socialLinksByKey.set(key, link);
        }
      });
  });

  return Array.from(socialLinksByKey.values()).sort((left, right) => {
...
export async function getHeroContentForSite(siteKey) {
  ...
  if (!connection) {
    return { activeHero: null, socialLinks: [] };
  }
  ...
  return {
    ...
    socialLinks: getScopedSocialLinks(activeHeroes, normalizedSiteKey),
  };
}
