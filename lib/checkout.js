...
export async function buildOrderDraft({ customer, cartProducts, siteKey }) {
  ...
  const store = await Store.findOne({}).select("shippingBaseCost shippingRatePerKm shippingFallbackCost").lean();

  const subtotal = orderItems.reduce(
    (runningTotal, item) => runningTotal + item.price * item.quantity,
    0
  );
  const shippingQuote = getShippingQuote(normalizedCustomer.city, store || {});
  ...
  return {
    ...
    locationName: orderLocation.locationName,
    orderItems,
    subtotal,
    shippingCost,
    total,
